...
const sendSwitchTransportRequest = async (transport: TransportType) =>
{
    ...
    try
    {
        const serverAddress = transport === TransportType.REMOTE ? selectedServer.value : undefined;
        await vibratorStore.switchTransport(transport, serverAddress);
...
{
    modalStore.close(ModalId.SERVER);
...
{
    modalStore.close(ModalId.SETTINGS);
    modalStore.open(ModalId.SERVER);
};
...
<template>
    <ion-modal :is-open="isOpen" @did-dismiss="handleDismiss">
        <div class="p-4 flex flex-col items-center text-center rounded-2xl modal-content">
            ...
            <div class="w-full space-y-6">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button @click="copyToClipboard(deviceInfo.macAddress, 'MAC Address')" class="copy-btn"
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button @click="copyToClipboard(deviceInfo.ipAddress, 'IP Address')" class="copy-btn"
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button @click="copyToClipboard(deviceInfo.deviceId, 'Device ID')" class="copy-btn"
                ...
                <div v-if="preferredTransport === TransportType.REMOTE" class="text-left">
                    <h3 class="text-md font-medium mb-3 text-gray-300">Remote Server</h3>
                    <ion-select v-model="selectedServer" interface="popover" class="transport-select">
                        <ion-select-option v-for="server in serverStore.servers" :key="server.url" :value="server.url">
                            {{ server.name }}
                        </ion-select-option>
                    </ion-select>
                    <button @click="handleAddCustomServerClick" class="mt-2 text-sm text-green-500 hover:text-green-400">
                        + Add Custom Server
                    </button>
                </div>

                <div class="text-left">
                    <h3 class="text-md font-medium mb-3 text-gray-300">Allowed Transports</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                            <span>Bluetooth</span>
                    ...
                    <p class="text-xs text-gray-400 mt-4">
                        By enabling remote transport, you allow others to connect to your device over the internet
                    </p>
                </div>
    ...
    <ServerModal :is-open="modalStore.isOpen(ModalId.SERVER)" @dismiss="() => modalStore.close(ModalId.SERVER)" @add="handleAddServer" />
</template>
